...
「更改配置」子菜单（`esc` 放弃并回退会话，`s` 保存并退出；`※即时` 项立即生效）：
...
持久化于 `state/customize.json`。编辑器为**缓冲式**：按 `s`（保存并退出）才写盘，ESC 放弃本次修改。

> 「更改配置」整个会话也是一个事务：期间的配置改动均为临时。`s`（保存并退出）和 `esc`
> （放弃并返回）是两个常驻基础按键 —— 按 `s` 提交，按 `esc` 回退本次会话的全部配置改动；
> 若回退牵涉到已重启的服务，会自动再重启一次以对齐回退后的配置。
